...
const Dashboard = () => {
  ...
  const stats = [
    {
      ...
      trendValue: '+5', // Placeholder trend for now
      icon: Mic2,
    },
    {
      title: 'Avg Fluency Score',
      value: loadingStats ? '—' : `${summary?.averageFluencyScore ?? 0}%`,
      trendValue: summary?.improvementTrend === 'improving' ? '+4%' : '-2%',
      icon: Activity,
    },
    {
      title: 'Latest Score',
  ...
  const chartData = trendData.length 
    ? trendData.map(d => ({ name: d.name, wpm: d.wpm }))
    : [
          ...
          <div className="relative z-10">
            ...
            <p className="text-white/80 text-sm mb-8 max-w-sm font-medium leading-relaxed">
              {topWeakSound
                ...
                : 'Based on your last session, practicing Plosive "P" Sounds will yield the highest fluency gain today.'}
            </p>
