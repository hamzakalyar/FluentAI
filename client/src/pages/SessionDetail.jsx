...
const SessionDetail = () => {
  ...
  const stutters = session.analysis?.stutters || [];
  const comparison = session.analysis?.comparison;
  const nlp = session.analysis?.nlp || {};
  const weakSounds = session.analysis?.weakSounds || [];
  const repetitions = session.analysis?.repetitions || [];
  const pauses = session.analysis?.pauses || [];
  const fillers = session.analysis?.fillers || [];
            ...
            {session.audioUrl && (
              <div className="bg-[var(--bg-elevated)] p-6 flex items-center gap-6 border-t border-[var(--border-subtle)]">
                ...
                <div className="flex-1 h-1.5 bg-[var(--bg-surface)] rounded-full relative overflow-hidden">
                  <div className="absolute inset-y-0 left-0 w-0 bg-[var(--accent)] rounded-full" />
                </div>
                <span className="text-[var(--text-muted)] text-xs font-mono">0:00 / {Math.floor(session.duration / 60)}:{(session.duration % 60).toFixed(0).padStart(2, '0')}</span>
              </div>
            )}
          </div>
