...
const Practice = () => {
  ...
  const fetchExercises = async (diff = difficulty) => {
    setLoading(true);
    setError(null);
    setCompletedIds([]);
    try {
      const res = await api.post('/practice/generate', { difficulty: diff, count: 5 });
      // Map backend exercise shape to component-expected shape
      const mapped = (res.data.exercises || []).map((ex, i) => ({
        id:            ex.id || String(i),
        num:           i + 1,
        title:         ex.title || `Exercise ${i + 1}`,
        tag:           ex.targetSounds?.[0] ? `/${ex.targetSounds[0]}/ Sound` : ex.tag || 'Mixed',
        difficulty:    ex.difficulty || diff,
      ...
      setError(msg || 'Failed to load exercises.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="lg:col-span-2 space-y-8">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-3">
              ...
              <p className="text-sm text-[var(--text-muted)] font-medium">Generating exercises for you…</p>
            </div>
          ) : error ? (
            <div className="bg-[var(--bg-surface)] border border-red-200 rounded-3xl p-10 text-center">
              <p className="text-sm text-red-500 font-bold mb-2">⚠️ {error}</p>
              <p className="text-xs text-[var(--text-muted)] mb-4">
                {error.includes('weak sounds') 
                  ? 'Complete a recording session first to identify your weak sounds, then come back here.'
                  : 'The practice service may be offline. Please try again.'}
              </p>
              <button onClick={() => fetchExercises()} className="h-9 px-5 rounded-xl bg-[var(--accent)] text-white font-bold text-xs">
                Try Again
              </button>
            </div>
          ) : (
