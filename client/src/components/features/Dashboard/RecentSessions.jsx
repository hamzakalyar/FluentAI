...
const RecentSessions = ({ sessions = [] }) => {
  return (
    <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl p-[20px_22px] shadow-sm flex flex-col h-full">
      {/* Card Header */}
      ...
      <div className="space-y-0 flex-1 flex flex-col">
        {sessions.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center py-8">
            <Mic2 size={32} className="text-[var(--text-muted)] mb-3 opacity-50" />
            <p className="text-[13px] font-bold text-[var(--text-primary)]">No sessions yet</p>
            <p className="text-[12px] text-[var(--text-muted)] mt-1 max-w-[200px]">Head to the Recording Studio to complete your first session.</p>
          </div>
        ) : (
          sessions.map((session, i) => (
            ...
            >
              ...
              <div className="flex-1 mx-3 min-w-0">
                 <p className="text-[13px] font-bold text-[var(--text-primary)] truncate">{session.title}</p>
                 <p className="text-[11px] font-mono text-[var(--text-tertiary)] uppercase tracking-tighter">
                    {session.date.toLocaleDateString()} — {Math.floor(session.duration/60)}:{String(session.duration%60).padStart(2,'0')}
                 </p>
              </div>
              ...
              <div className="flex items-center gap-2">
                 <span className={`px-2.5 py-0.5 rounded-full text-[12px] font-bold ${
                    session.fluencyScore >= 80 ? 'bg-[var(--accent-glow)] text-[var(--accent)]' :
                    session.fluencyScore >= 60 ? 'bg-amber-500/10 text-amber-500' :
