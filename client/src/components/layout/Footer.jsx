        ...
        <div className="flex flex-col lg:flex-row justify-between items-start gap-12 mb-16">
          ...
          <div className="flex gap-16 sm:gap-24">
            {SECTIONS.map(section => (
              <div key={section.title}>
                <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-teal-500 mb-5">
                  {section.title}
                </h4>
                <ul className="space-y-3">
                  {section.links.map(link => (
                    <li key={link.label}>
                      <Link to={link.to} className="text-slate-400 hover:text-white text-xs font-bold transition-colors">
                        {link.label}
                      </Link>
                    </li>
                  ))}
          ...
          <div className="bg-white/[0.03] border border-white/5 p-5 rounded-2xl max-w-[240px]">
             <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-white">Secure System</span>
             </div>
             <p className="text-[10px] text-slate-500 leading-normal font-medium mb-3">
                HIPAA compliant end-to-end encrypted architecture.
             </p>
             <div className="flex items-center gap-2 text-teal-500/80">
                <Lock size={10} />
                <span className="text-[9px] font-black uppercase tracking-widest">AES-256 Certified</span>
             </div>
          </div>
